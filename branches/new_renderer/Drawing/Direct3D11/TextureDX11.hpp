/*
 * OldSchoolHack GUI
 *
 * Copyright (c) 2010-2013 KN4CK3R http://www.oldschoolhack.de
 *
 * See license in OSHGui.hpp
 */

#ifndef OSHGUI_DRAWING_TEXTUREDX11_HPP
#define OSHGUI_DRAWING_TEXTUREDX11_HPP

#include <d3d11.h>

#include "ITexture.hpp"

namespace OSHGui
{
	namespace Drawing
	{
		class TextureDX11 : public ITexture
		{
		public:
			TextureDX11(ID3D11Device *device);
			virtual ~TextureDX11();
		
			ID3D11Texture2D* GetTexture();
			virtual bool IsLocked();

			virtual bool Create(const Drawing::SizeF &size);
		
			virtual void BeginUpdate();
			virtual void EndUpdate();
			
			void Clear();
			void Clear(const Drawing::PointF &point);
			void Clear(int x, int y);
			void Clear(Drawing::RectangleF &rect);
			virtual void Clear(int x, int y, int w, int h);

			void Fill(D3DCOLOR color);
			void Fill(const Drawing::PointF &point, D3DCOLOR color);
			void Fill(int x, int y, D3DCOLOR color);
			void Fill(Drawing::RectangleF &rect, D3DCOLOR color);
			virtual void Fill(int x, int y, int w, int h, D3DCOLOR color);

			void FillGradient(D3DCOLOR from, D3DCOLOR to, bool updown = true);
			void FillGradient(Drawing::RectangleF &rect, D3DCOLOR from, D3DCOLOR to, bool updown = true);
			virtual void FillGradient(int x, int y, int w, int h, D3DCOLOR from, D3DCOLOR to, bool updown = true);
			
			virtual void Rotate(int degrees);

			void Insert(const Drawing::PointF &point, Drawing::TextureDX11 *texture);
			virtual void Insert(int x, int y, Drawing::ITexture *texture);
			
		private:
			ID3D11Device *device;
			
			D3D11_MAPPED_TEXTURE2D lock;
			
			ID3D11Texture2D *texture;
		};
	}
}

#endif